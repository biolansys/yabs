...
<?  
   ...
   echo "<tr class=td_titulo0>",
        ...
        "</td></tr></table><br />";
  if ($output=='Table') { 
   $service_url = 'http://rest.ensembl.org/overlap/region/'.$specie.'/'.$region.'?'.$feature.';content-type=application/json' ;
   //echo $service_url;
   $response = file_get_contents($service_url);
   $json = new Services_JSON();
   ...
   if ($output=='gff3'){
   $service_url = 'http://rest.ensembl.org/overlap/region/'.$specie.'/'.$region.'?'.$feature.';content-type=application/gff3' ;
   $response = file_get_contents($service_url);
   //var_dump($response) ;
   $lines = explode("\n", $response);
...
<?     
   ...
   if ($output=='bed'){
    $service_url = 'http://rest.ensembl.org/overlap/region/'.$specie.'/'.$region.'?'.$feature.';content-type=application/bed' ;
    $response = file_get_contents($service_url);
   //var_dump($response) ;
   $lines = explode("\n", $response);
   $lines0=array_pop($lines);
